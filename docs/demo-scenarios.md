...
The MVP target is six repeatable labelled incident drills. The repository currently includes the deterministic database-pool saturation path used by unit tests and the web UI.
...
| API latency spike | `checkout-api` | elevated gateway latency | origin appears Top-3 RCA |
...
recommendation returns investigation steps and an approval-gated remediation action
```
